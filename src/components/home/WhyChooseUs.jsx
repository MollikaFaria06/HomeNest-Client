...
export default function WhyChooseUs() {
  ...
  return (
    <section className="bg-gradient-to-r from-teal-900 via-green-900 to-green-800 py-12 text-white">
      <div className="max-w-6xl mx-auto text-center px-4">
        <h2 className="text-3xl text-yellow-300 font-bold mb-6">
          Why Choose <span className="text-green-400">HomeNest?</span>
        </h2>
        <div className="grid md:grid-cols-3 gap-6">
          {reasons.map((r, i) => (
            <div
              key={i}
              className="p-6 bg-black/60 backdrop-blur-md shadow-md rounded-xl hover:shadow-lg transition"
            >
              <div className="flex justify-center mb-4">{r.icon}</div>
              <h3 className="font-semibold text-lg mb-2">{r.title}</h3>
              <p className="text-sm text-gray-200">{r.desc}</p>
            </div>
          ))}
        </div>
      </div>
